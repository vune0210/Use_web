...
import {ClassInvariantResp} from "./types/ClassInvariantResp";
import {StateEvolutionResp} from "./types/StateEvolutionResp";
import {ClassExtentResp} from "./types/ClassExtentResp";
import {CallstackResp} from "./types/CallstackResp";
import {EventResp} from "./types/EventResp";
...
class ViewApi extends Api {
    ...
    async getEvents(): Promise<EventResp> {
        return this.callApi<EventResp>(async () => {
            const res = await axios.get<EventResp>(this.path("/seq-diagram"));
            return res.data;
        });
    }
    ...
    async getClassInvariant(): Promise<ClassInvariantResp> {
        return this.callApi<ClassInvariantResp>(async () => {
            const res = await axios.get<ClassInvariantResp>(this.path("/invariant"));
            return res.data;
        });
    }
    ...
    async getClassExtent(className: string): Promise<ClassExtentResp> {
        return this.callApi<ClassExtentResp>(async () => {
            const res = await axios.get<ClassExtentResp>(this.path("/class-extent"), {
                params: {className},
            });
            return res.data;
        });
    ...
    async getStateEvolution(): Promise<StateEvolutionResp> {
        return this.callApi<StateEvolutionResp>(async () => {
            const res = await axios.get<StateEvolutionResp>(this.path("/state-evo"));
            return res.data;
        });
    }
