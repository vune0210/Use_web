...
set USE_JAR="%USE_HOME%\lib\use-backend.jar"

IF NOT EXIST %USE_JAR% (
	echo Cannot find USE executable. Please provide correct path to use-backend.jar.
	goto end
)

:runApp
java %VMARGS% -jar %USE_JAR% -nr -nogui %*
