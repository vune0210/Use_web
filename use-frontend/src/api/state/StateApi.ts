...
import {OclEvalResp} from "../view/types/OclEvalResp.ts";
...
class StateApi extends Api {
    ...
    async evalOclExpr(expr: string): Promise<OclEvalResp> {
        return this.callApi<OclEvalResp>(async () => {
            const res = await axios.post<OclEvalResp>(
                this.path("/ocl-eval"),
                {expr},
            );
            return res.data;
    ...
    async checkStateInvariants(): Promise<IApiResp> {
        return this.callApi<IApiResp>(async () => {
            const res = await axios.get<IApiResp>(this.path("/check-state-invariants"));
            return res.data;
        });
    }
}
